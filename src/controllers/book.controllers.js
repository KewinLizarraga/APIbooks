...
module.exports = {
  ...
  listByPage: async (req, res) => {
    try {
      ...
      res.status(200).json({ countBooks, perPage, pages, current, countBooksPage, books });
    } catch (error) {
      console.log('ERROR', error);
      res.status(500).json({ message: 'Internal Server Error.' });
    }
  ...
  create: async (req, res) => {
    try {
      ...
      if (oldBook) {
        return res.status(200).json({ message: 'Book is duplicated, enter another.' });
      }
  ...
  update: async (req, res) => {
    try {
      ...
      if (oldBook) {
        return res.status(200).json({
          message: 'Book is duplicated, enter another. Check ISBN or remove it to update'
        });
      }
